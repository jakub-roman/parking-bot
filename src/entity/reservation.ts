...
export class Reservation extends BaseEntity {
  ...
  static new(date: Date, user: User, spot: ParkingSpot ): Promise<Reservation> {
  ...
  static findForUser(username: string, date: Date): Promise<Reservation | null> {
    return Reservation
    .createQueryBuilder("reservation")
    .leftJoinAndSelect("reservation.user", "user")
    .leftJoinAndSelect("reservation.spot", "spot")
    .where({user: {name: username}, date: date})
    .getOne()
  }
  ...
  static async removeUserReservation(username: string, date: Date): Promise<string> {
    ...
    if (!reservation) throw new Error(`Can't find reservation on date ${date} with user ${username}`)
